...
var Tile=function(p_TileMngr,p_Col,p_Row,p_Size,p_Wall,p_Layer,p_Atlas,p_Variant){
	...
	this.m_AtlasY=p_Variant;
	this.m_Layer=p_Layer;
	this.m_Parent=p_TileMngr;
	this.m_Loaded=false;
...
function renderTile(p_Tile){  // closures consume way to much memory, using regular function for rendering instead
	...
	var tm=p_Tile.m_Parent;
	if(p_Tile.m_x+tm.WorldOffsetX>p_Tile.m_Canvas.width||p_Tile.m_x+tm.WorldOffsetX<-p_Tile.m_Size){return false;}
	if(p_Tile.m_y+tm.WorldOffsetY>p_Tile.m_Canvas.height||p_Tile.m_y+tm.WorldOffsetY<-p_Tile.m_Size){return false;}
	p_Tile.m_Ctx.drawImage(p_Tile.m_Atlas,
		(p_Tile.m_AtlasX*p_Tile.m_Size),
		(p_Tile.m_AtlasY*p_Tile.m_Size),
		p_Tile.m_Size,
		p_Tile.m_Size,
		p_Tile.m_x+tm.WorldOffsetX,
		p_Tile.m_y+tm.WorldOffsetY,
		p_Tile.m_Size,
		p_Tile.m_Size );  
...
var TileManager=function(p_Width,p_Height,p_StepSize){
	this.m_Tiles=[];
	this.m_MapWidth=0;//(p_Width===undefined)?800:p_Width;
	this.m_MapHeight=0;//(p_Height===undefined)?600:p_Height;
	this.m_Step=0;//(p_StepSize===undefined)?80:p_StepSize;
	...
	this.Init=function(p_Manifest,p_Ctx,p_Scene){
		...
		xThis.m_CanvasW=p_Ctx.canvas.width;
		xThis.m_CanvasH=p_Ctx.canvas.height;
		
		xThis.m_MapWidth=p_Manifest.hasOwnProperty('mapwidth')?p_Manifest.mapwidth:0;
		xThis.m_MapHeight=p_Manifest.hasOwnProperty('mapheight')?p_Manifest.mapheight:0;
		xThis.m_Step=p_Manifest.hasOwnProperty('step')?p_Manifest.step:0;
		
		var tileAtlas=p_Manifest.hasOwnProperty('tileatlas')?p_Manifest.tileatlas:false;
		if(tileAtlas){
			xThis.LoadTileAtlas(tileAtlas,function(){
				var mapSrc=p_Manifest.hasOwnProperty('bitmapsrc')?p_Manifest.bitmapsrc:false;
				if(mapSrc){
					xThis.LoadFromBitmap(mapSrc,function(){
	...
	this.Render=function(p_Ctx){
		...
		for(i=0;i<iC;i++){
			renderTile(xThis.m_Tiles[i]);
		}
	}
	this.NewTile=function(p_Col,p_Row,p_Wall,p_Layer,p_Variant){
		var variant=p_Variant===undefined?0:p_Variant;
		var nt=new Tile(xThis,p_Col,p_Row,xThis.m_Step,p_Wall,p_Layer,xThis.m_TileAtlas,variant);
	...
	this.GetTileFromWorldPos=function(p_x,p_y,p_w,p_h){
		var ts=xThis.m_Tiles;
		var i,iC=ts.length;
		var s=xThis.m_Step;
		for(i=0;i<iC;i++){
			var iso=ts[i].GetWorldPosition();
			var x=iso[0];
			var y=iso[1];
			if(p_x>=x&&p_x<x+ts[i].m_SpriteSize[0]){
				if(p_y>=y&&p_y<y+ts[i].m_SpriteSize[1]){
					return ts[i];
				}
			}
		}
	...
	this.GetTilesInArea=function(p_x,p_y,p_w,p_h){
		var ts=xThis.m_Tiles;
		var i,iC=ts.length;
		var s=xThis.m_Step;
		var ret=[];
		for(i=0;i<iC;i++){
			...
			var x=iso[0];
			var y=iso[1];
			var w=ts[i].m_SpriteSize[0];
			var h=ts[i].m_SpriteSize[1]
			if((p_x>=x&&p_x<x+w)||(p_x+p_w>=x&&p_x+w<x+w)){
				if((p_y>=y&&p_y<y+h)||(p_y+p_h>=y&&p_y+h<y+h)){
					ret.push(ts[i]);
				}
			}
		}
	...
	this.WallInArea=function(p_x0,p_y0,p_w,p_h){
		var tiles=xThis.GetTilesInArea(p_x0,p_y0,p_w,p_h);
		var i,iC=tiles.length;
		var wallFilter=[0,16,17,18];
		
		for(i=0;i<iC;i++){
			if(wallFilter.indexOf(tiles[i].m_Wall)==-1){
				return true;
			}
		}
		return false;
	...
	this.LoadFromBitmap=function(p_Src,p_Callback){
		...
		mapdata.onload=function(){
			...
			var numVariants=xThis.m_TileAtlas.height/tileS;
			for(i=0;i<iC;i+=step){
				var r=parseInt((i)/w);
				...
				xThis.NewTile(col,row,wall,(isWall)?0:2,variant);
